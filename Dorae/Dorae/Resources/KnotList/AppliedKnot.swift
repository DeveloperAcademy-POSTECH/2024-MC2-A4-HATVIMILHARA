//
//  AppliedKnot.swift
//  Dorae
//
//  Created by 지영 on 5/20/24.
//

import Foundation

let 항아리매듭 = 응용(매듭이름: .항아리매듭, 매듭이미지: .항아리매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 3)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 3)),
    .기본(매듭: 기본(매듭이름: .세벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 지게매듭 = 응용(매듭이름: .지게매듭, 매듭이미지: .지게매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .세벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 왕비국화매듭 = 응용(매듭이름: .왕비국화매듭, 매듭이미지: .왕비국화매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭, 개수: 3)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭, 개수: 3)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .네벌국화)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 육립매듭 = 응용(매듭이름: .육립매듭, 매듭이미지: .육립매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .거꾸로나비매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .네벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 꽃육립매듭 = 응용(매듭이름: .꽃육립매듭, 매듭이미지: .꽃육립매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 4)),
    .기본(매듭: 기본(매듭이름: .네벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 4)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 공작매듭 = 응용(매듭이름: .공작매듭, 매듭이미지: .공작매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .나비매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭, 개수: 9)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭, 개수: 9)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .장구매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 쌍공작매듭 = 응용(매듭이름: .쌍공작매듭, 매듭이미지: .쌍공작매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .세벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .네벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .가락지매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .세벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])

let 스타매듭 = 응용(매듭이름: .스타매듭, 매듭이미지: .스타매듭, 하위매듭리스트: [
    .기본(매듭: 기본(매듭이름: .도래매듭)),
    .기본(매듭: 기본(매듭이름: .세벌국화)),
    .기본(매듭: 기본(매듭이름: .도래매듭, 개수: 2)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .다섯벌국화)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .다섯벌국화)),
    .기본(매듭: 기본(매듭이름: .생쪽매듭)),
    .기본(매듭: 기본(매듭이름: .도래매듭))
])


let 응용매듭모음: [응용] = [
    항아리매듭,
    지게매듭,
    왕비국화매듭,
    육립매듭,
    꽃육립매듭,
    공작매듭,
    쌍공작매듭,
    스타매듭
]
